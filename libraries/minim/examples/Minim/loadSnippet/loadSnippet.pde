/**
  * This sketch demonstrates how to use the <code>loadSnippet</code> method of <code>Minim</code>. The <code>loadSnippet</code> 
  * method allows you to specify the file you want to load with a <code>String</code>. Unlike with <code>loadFile</code> 
  * and <code>loadSample</code>, you are not able to specify a buffer size because an <code>AudioSnippet</code> doesn't 
  * give you access to the samples as they are played. 
  * <p>
  * Minim is able to load wav files, au files, aif files, snd files, and mp3 
  * files. When you call <code>loadSnippet</code>, if you just specify the filename it will try to load the file from 
  ...
  * expressed in milliseconds instead of microseconds. One of the limitations of <code>AudioSnippet</code> is 
  * that you do not have access to the audio samples as they are played. However, you are spared all
  * of the overhead associated with making samples available. An <code>AudioSnippet</code> is a good choice 
  ...
  * method of any <code>AudioSnippet</code>'s you have received from <code>loadSnippet</code>.
  */
